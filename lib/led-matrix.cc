...
RGBMatrix::RGBMatrix(GPIO *io, int rows, int columns, int chained_displays, int parallel_displays)
  : rows_(rows),
    columns_(columns),
    chained_displays_(chained_displays),
...
FrameCanvas *RGBMatrix::CreateFrameCanvas() {
  FrameCanvas *result =
    new FrameCanvas(new internal::Framebuffer(rows_, columns_ * chained_displays_,
                                              parallel_displays_));
